...
### 2.1 프로젝트 초기화
- [x] Next.js 프로젝트 생성 (`frontend/`, App Router, TypeScript, Tailwind CSS)
- [x] 기본 디렉토리 구조 생성 (`components/`, `lib/`, `types/`)

### 2.2 인증 UI
- [x] `lib/auth.ts` - JWT 토큰 저장/조회/삭제 (localStorage), 인증 상태 관리
- [x] `app/login/page.tsx` - 로그인 페이지 (이메일, 비밀번호 입력)
- [x] `app/signup/page.tsx` - 회원가입 페이지 (이메일, 비밀번호, 이름 입력)
- [x] 인증 API 연동 (signup, login)
- [x] 로그인/회원가입 성공 시 → 리마인더 홈(`/`)으로 리다이렉트
- [x] 비로그인 상태에서 보호된 페이지 접근 시 → `/login`으로 리다이렉트 (AuthGuard)
- [x] 401 응답 시 → 토큰 삭제 후 `/login`으로 리다이렉트

### 2.3 타입 정의
- [x] `types/reminder.ts` - Reminder, ReminderList, Priority 타입
- [x] `types/auth.ts` - Member, AuthResult 타입

### 2.4 API 클라이언트
- [x] `lib/api.ts` - fetch 래퍼 (baseURL, JSON 헤더, JWT 토큰 자동 포함, 401 핸들링)
- [x] 인증 API 함수 (signup, login)
- [x] 목록 API 함수 (getLists, createList, updateList, deleteList)
- [x] 리마인더 API 함수 (getReminders, createReminder, updateReminder, deleteReminder, toggleReminder)

### 2.5 레이아웃
- [x] `app/layout.tsx` - 루트 레이아웃 (2-패널: Sidebar + Main Content)
- [x] Sidebar 컴포넌트 - 사용자 목록
- [x] 목록 선택 상태 관리 (pathname 기반 active 표시)

### 2.6 리마인더 목록 표시
- [x] `app/lists/[id]/page.tsx` - 목록별 리마인더 표시
- [x] ReminderItem 컴포넌트 - 체크박스, 제목, 우선순위, 마감일

### 2.7 검증
- [x] TypeScript 타입 체크 통과
- [ ] E2E 수동 검증 (백엔드 + 프론트엔드 동시 실행 후 확인)
...
### 3.1 리마인더 추가
- [x] "+ 리마인더 추가" 버튼
- [x] 인라인 입력 폼 (Enter로 생성)

### 3.2 리마인더 편집
- [x] 항목 클릭 시 인라인 편집 모드
- [x] 제목, 메모, 마감일(date picker), 우선순위, 플래그 편집
